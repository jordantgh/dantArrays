...
def usage_example():
    ...
    price_data = np.random.normal(loc=[[100], [50], [200]], scale=[[10], [5], [20]], size=(3, 100))
    ...
    stocks = MetadataArray(data=price_data, metadata_class=StockMetadata, major_axis=0)
    
    # Set metadata for each stock
    stocks.update_metadata(0, symbol="AAPL", company_name="Apple Inc.", sector="Technology")
    stocks.update_metadata(1, symbol="MSFT", company_name="Microsoft Corp.", sector="Technology")
    stocks.update_metadata(2, symbol="AMZN", company_name="Amazon.com Inc.", sector="Consumer Cyclical")
    ...
    class ImageMetadata(BaseModel):
        ...
        brightness: ComputedField = data_based_field(lambda x: float(np.mean(x)))
    ...
    image_dataset = MetadataArray(data=images, metadata_class=ImageMetadata, major_axis=0)
    
    # Add metadata for some images
    image_dataset.update_metadata(0, label="cat", confidence=0.95, tags=["pet", "mammal"])
    image_dataset.update_metadata(1, label="dog", confidence=0.87, tags=["pet", "mammal"])
    image_dataset.update_metadata(2, label="car", confidence=0.92, tags=["vehicle"])
    ...
    print(f"Experiment data shape: {experiment_data.shape} (10 experiments, 1000 measurements)")
    
    # Create a MetadataArray for experiment data
    experiments = MetadataArray(data=experiment_data, metadata_class=ExperimentMetadata, major_axis=0)
    ...
    for i in range(3):
        ...
        print(f"    Conditions: {meta.temperature_c}°C, {meta.pressure_kpa} kPa")
    ...
    #########################################################################
    print("\n\n" + "-"*80)
    print("EXAMPLE 4: MULTI-DIMENSIONAL TENSOR WITH FEATURE METADATA".center(80))
    print("-"*80)
    ...
    print(f"Feature dataset shape: {feature_data.shape} (100 samples, 20 features)")
    
    # Create a MetadataArray with column metadata (each column is a feature)
    dataset = MetadataArray(data=feature_data, metadata_class=FeatureMetadata, major_axis=1)
    
    # Set metadata for features
    dataset.update_metadata(0, name="age", description="Age in years", importance=0.8)
    dataset.update_metadata(1, name="income", description="Annual income", importance=0.9)
    dataset.update_metadata(2, name="category", description="Product category", is_categorical=True, data_type="categorical")
    dataset.update_metadata(3, name="height", description="Height in cm", importance=0.4)
    dataset.update_metadata(4, name="weight", description="Weight in kg", importance=0.5)
